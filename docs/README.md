...
boilerplate, libraries and strengths to quickly get you going when creating a new plugin.
...
- Stable releases as: `StrengthPlugin-1.0.0.jar`
- Testing releases as: `StrengthPlugin-1.0.0-RC-X.jar`
- Development releases as: `StrengthPlugin-1.0.0-SNAPSHOT-X.jar`
...
4. #### Change [Project Name](./settings.gradle.kts)
   Replace all occurances of `StrengthPlugin` with your new plugin name. Don't forget to rename the main
   class [StrengthPlugin.java](./src/main/java/com/github/StrengthUser/StrengthPlugin/StrengthPlugin.java) to the same
   value.
   ```kotlin
...
5. #### Change [build.gradle.kts](./build.gradle.kts)
    1. ##### Change Plugin Info
       > The final package path for your plugin will end up being something like `io.github.darksaid98.strengthplugin`. It's made up of these components `io.github.<USERNAME>.<PLUGINNAME>` where `<USERNAME>` is your github name in lowercase, and `<PLUGINNAME>` is added by `rootProject.name` lowercased.
